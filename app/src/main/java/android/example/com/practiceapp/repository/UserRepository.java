package android.example.com.practiceapp.repository;
...
public class UserRepository {
    ...
    private UserRepository(FirebaseFirestore firestore, FirebaseStorage storage) {
    ...
    public synchronized static UserRepository getInstance(FirebaseFirestore firestore, FirebaseStorage storage) {
        Log.d(TAG, "Getting the repository");
        if (sInstance == null) {
            synchronized (LOCK) {
                sInstance = new UserRepository(firestore, storage);
    ...
    public Map<String, MutableLiveData<String>> loadCounters(String email) {
        final Map<String, MutableLiveData<String>> data = new HashMap<>();
        DocumentReference documentReference = countersRef.document(email);
        documentReference.addSnapshotListener(new EventListener<DocumentSnapshot>() {
            @Override
            public void onEvent(@Nullable DocumentSnapshot snapshots, @Nullable FirebaseFirestoreException e) {
                MutableLiveData<String> posts = new MutableLiveData<>();
                MutableLiveData<String> followers = new MutableLiveData<>();
                MutableLiveData<String> followings = new MutableLiveData<>();
                if (e != null) {
                    Log.w(TAG, "Failed with ", e.fillInStackTrace());
                    return;
                } if (snapshots.contains(POSTS)) {
                    posts.setValue(snapshots.get(POSTS).toString());
                    data.put(POSTS, posts);
                } if (snapshots.contains(FOLLOWERS)) {
                    followers.setValue(snapshots.get(FOLLOWERS).toString());
                    data.put(FOLLOWERS, followers);
                } if (snapshots.contains(FOLLOWS)) {
                    followings.setValue(snapshots.get(FOLLOWS).toString());
                    data.put(FOLLOWS, followings);
                }
            }
        });
        return data;
